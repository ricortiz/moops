...
extern "C"
{
                            ...
                            float delta)
    {
        ...
        //register memory
        cudaError_t err;
        err =   cudaHostRegister(positions, /*getAlignedSize(*/sizeof(float) * 6 * NUM_BODIES/*)*/, cudaHostRegisterMapped);
        checkError(err);
        err =   cudaHostRegister(gpuVelocities, /*getAlignedSize(*/sizeof(float) * 3 * NUM_BODIES/*)*/, cudaHostRegisterMapped);
        checkError(err);
        err =   cudaHostRegister(target_list, /*getAlignedSize(*/sizeof(unsigned int) * 2 * NUM_LEAF_NODES/*)*/, cudaHostRegisterMapped);
        checkError(err);
        err =   cudaHostRegister(num_interaction_pairs, /*getAlignedSize(*/sizeof(unsigned int) * NUM_LEAF_NODES/*)*/, cudaHostRegisterMapped);
        checkError(err);
        ...
        cudaMemcpy(gpuVelocities, (float *)dvelocities, 1024*sizeof(float), cudaMemcpyDeviceToHost);
